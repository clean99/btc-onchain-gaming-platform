...
return i-1;}
this.init=function()
{ctx=g('canvas').getContext('2d');ctx.translate(100,150);ctx.lineJoin=ctx.lineCap='round';ctx.globalAlpha=0.9;ctx.lineWidth=0.7;ctx.strokeStyle='rgba(0, 0, 0, 0)';}
...
function getBlockColor(type, intensity, lightness) {
    ...
    switch (type) {
        ...
        case 6:
            return '#D86929'; // 更改为 Hex 颜色
        case 20:
            return '#D86929'; // 示例: 灰色（您可以自定义这个颜色）
        case 30:
            return '#D86929'; 
        default:
            return '#D86929'; 
    }
    console.log(`Type: ${type}, Color: ${color}`); // Debugging output
    return color;
}
...
return true;}};var MENU_QUOTES=[""];var UI=new function()
...
{Game.paused?Control.resumeGame():Control.pauseGame();};g('but_resume').onclick=Control.resumeGame;

g('but_restart').onclick=g('but_restart2').onclick=Control.restartGame;g('but_quit').onclick=function()
